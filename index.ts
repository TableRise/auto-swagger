...
const renderMiddleware = process.env.NODE_ENV === 'dev';
...
export function buildRouter(routes: routeInstance[], router: Router): Router {
  routes.forEach((route) => {
    ...
    if (!route.controller && route.options.middlewares.length && !renderMiddleware) return router[route.method](route.path);
    if (!route.controller && route.options.middlewares.length && renderMiddleware) return router[route.method](route.path, ...route.options.middlewares);
    ...
    if (route.controller && route.options.middlewares.length && !renderMiddleware) return router[route.method](route.path, route.controller);
    if (route.controller && route.options.middlewares.length && renderMiddleware) return router[route.method](route.path, ...route.options.middlewares, route.controller);
  });
